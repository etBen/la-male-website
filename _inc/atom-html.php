<?php
  ...
  class myAtomParser
  {
	...
	I've updated the way the HTML is outputt so that it's a simple list (<li></li>)
	...
	**************************************************************************************************/
    ...
    function display_entry($data, $parent)
    {
	  ...
	  // Fix the links to Twitter Search
	  $CONTENT = str_replace("/search?q=", "http://search.twitter.com/search?q=", $CONTENT);
	
      if($CONTENT) {
        $this->retval .=  "<li><p>" . stripslashes($CONTENT) . "</p><br />\n\n";
      } elseif($SUMMARY) {
        $this->retval .=  "<li><p>" . stripslashes($SUMMARY) . "</p><br />\n\n";
      }
	  
	  if($AUTHOR) {
        $this->retval .=  "<p>Twitter: <a href='" . stripslashes($AUTHOR['URI']) . "'>" . stripslashes($AUTHOR['NAME']) . "</a></p></li>\n\n";
      }
	
    }
	/**************************************************************************************************/
	/* End - Modified by Ryan Carson (ryan@carsonified.com)												  */
	/**************************************************************************************************/
